...
interface DailyJourneyModalProps {
  game: Game;
  onSaveToSlipLine?: (
    lineLabel: string,
    numbers: SlipNumber[],
...
export function DailyJourneyModal({
  game,
  onSaveToSlipLine
}: DailyJourneyModalProps) {
  const {
    isOpen,
    currentStep,
    totalSteps,
  ...
  const handleApplyCompleted = () => {
    ...
    if (onSaveToSlipLine) {
      onSaveToSlipLine("D", formattedNumbers, journeyCommentary || undefined);
    }
    resetJourney();
  };
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/60 bg-muted/20">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-8 h-8 rounded-xl bg-gradient-to-br from-rose-600 via-orange-500 to-amber-500 text-white font-black text-xs shadow">
              ⭐️
            </div>
            <div>
        ...
        {showCancelConfirm && (
          <div className="absolute inset-0 z-20 bg-background/90 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center space-y-4">
            ...
            <h4 className="text-lg font-bold text-foreground">
              Dừng hành trình Daily Journey?
            </h4>
            <p className="text-xs text-muted-foreground max-w-sm">
              Bạn đang trả lời giữa chừng. Nếu thoát bây giờ, bạn có thể quay lại sau để chơi tiếp. Số đã mở của ngày hôm nay sẽ được giữ nguyên.
            </p>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
          ...
          {isCompleted ? (
            <LuckyCompleteView
              ...
              journeyCommentary={journeyCommentary || "Bạn đã hoàn thành hành trình Số phận ngày hôm nay!"}
